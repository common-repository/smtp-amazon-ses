	...
	public function saveSettings() {
		try {
			...
			if ( isset( $_POST['settings'] ) ) {
				...
				wp_send_json_success( array( 'mess' => __( 'Settings saved.', 'smtp-amazon-ses' ) ) );
			}
			wp_send_json_error( array( 'mess' => __( 'Settings Failed.', 'smtp-amazon-ses' ) ) );
		} catch ( \Exception $ex ) {
		...
		try {
			...
			if ( isset( $_POST['emailAddress'] ) ) {
				...
				if ( ! is_email( $emailAddress ) ) {
					wp_send_json_error( array( 'mess' => __( 'Invalid email format!', 'smtp-amazon-ses' ) ) );
				}

				$headers      = "Content-Type: text/html\r\n";
				$subjectEmail = __( 'YaySMTP - Test email was sent successfully!', 'smtp-amazon-ses' );
				$html         = __( 'Yay! Your test email was sent successfully! Thanks for using <a href="https://yaycommerce.com/yaysmtp-wordpress-mail-smtp/">YaySMTP</a><br><br>Best regards,<br>YayCommerce', 'smtp-amazon-ses' );

				if ( ! empty( $emailAddress ) ) {
					$sendMailSucc = wp_mail( $emailAddress, $subjectEmail, $html, $headers );
					if ( $sendMailSucc ) {
						Utils::setYaySmtpSetting( 'succ_sent_mail_last', true );
						wp_send_json_success( array( 'mess' => __( 'Email has been sent.', 'smtp-amazon-ses' ) ) );
					} else {
				...
				wp_send_json_error( array( 'mess' => __( 'Email Address is not empty.', 'smtp-amazon-ses' ) ) );
			}
			wp_send_json_error( array( 'mess' => __( 'Error send mail!', 'smtp-amazon-ses' ) ) );
		} catch ( \Exception $ex ) {
			LogErrors::getMessageException( $ex, true );
		} catch ( \Error $ex ) {
			LogErrors::getMessageException( $ex, true );
			...
			if ( isset( $_POST['params'] ) ) {
				...
				wp_send_json_success(
					array(
						...
						'mess'            => __( 'Successful', 'smtp-amazon-ses' ),
					)
				);
			}
			wp_send_json_error( array( 'mess' => __( 'Failed.', 'smtp-amazon-ses' ) ) );
		} catch ( \Exception $ex ) {
			LogErrors::getMessageException( $ex, true );
		} catch ( \Error $ex ) {
			LogErrors::getMessageException( $ex, true );
	...
	public function setYaySmtpEmailLogSetting() {
		try {
			...
			if ( isset( $_POST['params'] ) ) {
				...
				wp_send_json_success(
					array(
						'mess' => __( 'Save Settings Successful', 'smtp-amazon-ses' ),
					)
				);
			}
			wp_send_json_error( array( 'mess' => __( 'Save Settings Failed.', 'smtp-amazon-ses' ) ) );
		} catch ( \Exception $ex ) {
			LogErrors::getMessageException( $ex, true );
		} catch ( \Error $ex ) {
			LogErrors::getMessageException( $ex, true );
	...
	public function deleteEmailLogs() {
		try {
			...
			if ( isset( $_POST['params'] ) ) {
				...
				if ( ! $deleted ) {
					wp_send_json_error( array( 'mess' => __( 'Something wrong, Email logs not deleted', 'smtp-amazon-ses' ) ) );
				}

				wp_send_json_success(
					array(
						'mess' => __( 'Delete successful.', 'smtp-amazon-ses' ),
					)
				);
			}
			wp_send_json_error( array( 'mess' => __( 'No email log id found.', 'smtp-amazon-ses' ) ) );
		...
		try {
			...
			if ( isset( $_POST['params'] ) ) {
				...
				if ( ! empty( $resultQuery ) ) {
					...
					wp_send_json_success(
						array(
							'mess' => __( 'Get email log #' . $id . ' successful.', 'smtp-amazon-ses' ),
							'data' => $resultArr,
						)
					);
				} else {
					wp_send_json_error( array( 'mess' => __( 'No email log found.', 'smtp-amazon-ses' ) ) );
				}
			}
			wp_send_json_error( array( 'mess' => __( 'No email log id found.', 'smtp-amazon-ses' ) ) );
